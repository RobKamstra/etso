defmodule Etso.Cache.CleaningStrategy.LeastRecentlyWritten do
  ...
  def cleanup(strategy_state, repo, _schema, cache_entry_repo) do
    ...
    new_queries =
      Map.new(strategy_state.queries, fn {query_id, query_stats} ->
        results_count = query_stats.results_count
        cache_entry_count = query_stats.cache_entry_count

        average_results_per_entry = results_count / cache_entry_count

        amount_to_be_freed_query =
          :math.ceil(query_stats.results_count * strategy_state.free_percentage)
